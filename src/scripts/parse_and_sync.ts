...
async function main() {
    ...
    try {
        ...
        console.log(`[ParseAndSync] Total events parsed: ${parseResult.events.length}`);
        console.log(`[ParseAndSync] Output file: ./output/${parseResult.outputFile}`);
        console.log(`[ParseAndSync] Screenshot: ./output/${parseResult.screenshotPath}`);
        console.log('');
        ...
        console.log(`[ParseAndSync] Found ${activeEvents.length} active events out of ${parseResult.events.length} total`);
        ...
        console.log(`[ParseAndSync] Using sync timestamp: ${syncTimestamp.toISOString()}`);
        console.log('');

        const result = await mondayService.syncActiveEvents(activeEvents, syncTimestamp);
        ...
        for (const detail of result.details) {
            ...
            switch (detail.status) {
                case 'updated':
                    console.log(`[ParseAndSync] ✓ ${eventId} - Updated (Item: ${detail.mondayItemId})`);
                    break;
                case 'skipped':
                    console.log(`[ParseAndSync] ⚠ ${eventId} - Skipped: ${detail.message}`);
                    break;
                case 'error':
