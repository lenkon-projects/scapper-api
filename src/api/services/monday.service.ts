...
class MondayService {
    ...
    private loadConfig(): MondayConfig {
        return {
            ...
            capacityColumn: process.env.MONDAY_COM_CAPACITY_COLUMN || 'numeric_mkxst6mx',
            ticketsSoldColumn: process.env.MONDAY_COM_TICKETS_SOLD_COLUMN || 'numeric_mkxsf3c8',
            updateDateColumn: process.env.MONDAY_COM_UPDATE_DATE_COLUMN || 'date_mky2adca'
        };
    }
    ...
    private validateConfig(): void {
        ...
        if (!this.config.boardId) {
            throw new Error('Missing required environment variable: MONDAY_COM_BOARD_ID');
        }
    }
     ...
     * @param eventId - The event ID to search for (will be prefixed with "OZ-")
     * @returns Monday.com item if found, null otherwise
     */
    public async searchItemByEventId(eventId: string): Promise<MondayItem | null> {
        const searchValue = `OZ-${eventId}`;
        ...
        try {
            const response = await this.client.post<MondayApiResponse<ItemsSearchResponse>>('', {
                query,
                variables
            });
            ...
            if (axios.isAxiosError(error)) {
                ...
                throw new MondayApiError(`Failed to search item: ${message}`, statusCode);
            }
            throw error;
        }
    }
     ...
     */
    public async updateItem(itemId: string, event: Event, timestamp: Date): Promise<boolean> {
        ...
        try {
            const response = await this.client.post<MondayApiResponse<ChangeMultipleColumnValuesResponse>>('', {
                query: mutation,
                variables
            });
            ...
            if (axios.isAxiosError(error)) {
                const statusCode = error.response?.status || 500;
                const message = error.response?.data?.error_message || error.message;
                throw new MondayApiError(`Failed to update item: ${message}`, statusCode);
            }
            throw error;
        }
    }
     ...
     */
    public async syncEvent(event: Event, timestamp: Date): Promise<SyncDetail> {
        const eventId = event.eventId || 'unknown';

        try {
            // Search for item
            const item = await this.searchItemByEventId(eventId);
     ...
     */
    public async syncActiveEvents(events: Event[], timestamp: Date): Promise<SyncResult> {
        ...
        for (const event of events) {
            ...
            const detail = await this.syncEvent(event, timestamp);
